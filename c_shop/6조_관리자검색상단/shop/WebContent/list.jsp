...
int nowpage = ((Integer)request.getAttribute("maxpage")).intValue();
...
<body>
  ...
  <section>
	...
	<c:if test="${page>1 }"><a href="list.do?searchFlag=${searchFlag }&opt=${opt}&search=${search}&page=${page-1}"><li class="prev"></li></a></c:if>
	<!--  순차적 페이지 출력 -->
	...
	<c:if test="${page<maxpage }"><a href="list.do?searchFlag=${searchFlag }&opt=${opt }&search=${search }&page=${a }"><li class="next"></li></a></c:if>
	<!-- 마지막 페이지 이동 -->
	<a href="list.do?searchFlag=${searchFlag }&opt=${opt }&search=${search }&page=${maxpage }"><li class="last"></li></a>
	</c:when>
	...
	<!-- 첫페이지 -->	
     ...
     <!-- 이전페이지 -->
     <c:if test="${page<=1 }"><li class="prev"></li></c:if>
     <c:if test="${page>1 }"><a href="list.do?page=${page-1 }"><li class="prev"></li></a></c:if>
     <!--순차적 페이지 이동 -->
     <c:forEach var="a" begin="${startpage }" end="${endpage }" step="1">
     	<c:choose>
     		<c:when test="${a==page }">
			...
			<c:when test="${a!=page }">
				...
				<li class="num"><div>${a }</div></li></a>
			</c:when>
		</c:choose>
	</c:forEach>
      <!-- 다음 페이지로 이동 -->
      <c:if test="${page>=maxpage }"><li class="next"></li></c:if>
      <c:if test="${page<maxpage }"><a href="list.do?searchFlag=${searchFlag }&opt=${opt}&search=${search}&page=${page+1}"><li class="next"></li></a></c:if>
      <!-- 마지막 페이지 이동휘 -->
      <a href="list.do?searchFlag=${searchFlag }&opt=${opt }&search=${search }&page=${maxpage }"><li class="last"></li></a>
	</c:otherwise>
	</c:choose>
