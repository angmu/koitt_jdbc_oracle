...
<%
	...
	//int listcount = Integer.parseInt((String)request.getAttribute("listcount")); -> 이것은 에러!!!
...
<body>
  ...
  <section>
    ...
    <table>
      ...
      <tr>
        <td><span class="table-notice">${dt.bid }</span></td>
        <td class="table-title">
        <a href="content_view.do?bId=${dt.bid }">
        <c:forEach begin="1" end="${dt.bindent }">▶</c:forEach>
        ${dt.btitle }</a></td>
        <td>${dt.bdate }</td>
        <td>${dt.bhit }</td>
      </tr>
