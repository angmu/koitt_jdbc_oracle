...
<%
	...
	//request로 들어가면 객체형 integer로 형변환해서 들어가기때문에 String으로 바꾸는건 안되지만 1의 경우로는 변경가능
	//1의경우 int listcount=Integer.parseInt(request.getAttribute("listcount").toString()); >>괜찮음.
	//2의경우 int listcount=Integer.parseInt((String)request.getAttribute("listcount")); >>에러
...
<body>
  ...
  <section>
    <h1>NOTICE</h1>
    <div class="wrapper">
      <form action="list.do" name="search_box" method="post">
    ...
    <table>
      ...
      <c:forEach var="dto" items="${list }">
	      <tr>
	        ...
	        <c:forEach begin="1" end="${dto.bIndent }">└</c:forEach>${dto.bTitle }</td></a>
	        <td>${dto.bDate }</td>
	        <td>${dto.bHit }</td>
	      </tr>
      </c:forEach>
    	...
    	<c:when test="${searchflag!=null }">
		    ...
		    <ul class="page-num">
		      ...
		      <c:forEach var="a" begin="${startpage }" end="${endpage }" step="1">
			      ...
			      <c:when test="${a!=page }">
				      <li class="num">
				        <a href="list.do?searchflag=${searchflag }&opt=${opt }&search=${search }&page=${a }"><div>${a }</div></a>
				      </li>
			      </c:when>
			      </c:choose>
		      </c:forEach>
	    	  ...
	    	  <!-- flag가 없을경우 -->
			    ...
			    <ul class="page-num">
			      ...
			      <c:forEach var="a" begin="${startpage }" end="${endpage }" step="1">
				      ...
				      <c:when test="${a!=page }">
					      <li class="num">
					        <a href="list.do?page=${a }"><div>${a }</div></a>
					      </li>
				      </c:when>
				      </c:choose>
			      </c:forEach>
